...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagWatering)
    {
        info->event[info->num] = EVENT_Watering;
        info->num++;
        dataPoints->valueWatering = gizStandardDecompressionValue(Watering_BYTEOFFSET,Watering_BITOFFSET,Watering_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
        
    if(0x01 == issuedData->attrFlags.flagAutomatic)
    {
        info->event[info->num] = EVENT_Automatic;
        info->num++;
        dataPoints->valueAutomatic = gizStandardDecompressionValue(Automatic_BYTEOFFSET,Automatic_BITOFFSET,Automatic_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valueWatering != cur->valueWatering)
    {
        GIZWITS_LOG("valueWatering Changed\n");
        ret = 1;
    }
    if(last->valueAutomatic != cur->valueAutomatic)
    {
        GIZWITS_LOG("valueAutomatic Changed\n");
        ret = 1;
    }
    if(last->valueWarning != cur->valueWarning)
    {
        GIZWITS_LOG("valueWarning Changed\n");
        ret = 1;
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizStandardCompressValue(Watering_BYTEOFFSET,Watering_BITOFFSET,Watering_LEN,(uint8_t *)devStatusPtr,dataPoints->valueWatering);
    gizStandardCompressValue(Automatic_BYTEOFFSET,Automatic_BITOFFSET,Automatic_LEN,(uint8_t *)devStatusPtr,dataPoints->valueAutomatic);
    gizStandardCompressValue(Warning_BYTEOFFSET,Warning_BITOFFSET,Warning_LEN,(uint8_t *)devStatusPtr,dataPoints->valueWarning);
    ...
    devStatusPtr->valueVoltage = gizY2XFloat(Voltage_RATIO,  Voltage_ADDITION, dataPoints->valueVoltage); 
